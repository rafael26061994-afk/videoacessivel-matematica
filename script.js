...
let moves = 0;
let fontScale = 1.0; // Escala inicial (100%)
...
function renderPond3D() {
    ...
    currentState.forEach((state, index) => {
        ...
        shadow.className = 'pad-shadow';
        pad3D.appendChild(shadow);

        const topFace = document.createElement('div');
        topFace.className = 'pad-face-top';

        if (state === 'R') {
            // Sapo Verde olhando para a DIREITA
            topFace.innerHTML = `
                <div class="frog-illustration-container" id="frog-${index}" style="transform: scaleX(1) rotateX(-24deg) translateZ(12px);">
                    <img src="sapo-verde.png" alt="Sapo Verde virado para a direita" class="frog-img">
                </div>
            `;
        } else if (state === 'L') {
            // Sapo Amarelo olhando para a ESQUERDA
            topFace.innerHTML = `
                <div class="frog-illustration-container" id="frog-${index}" style="transform: scaleX(1) rotateX(-24deg) translateZ(12px);">
...
function handlePadClick(index) {
    ...
    // Se tentar um movimento inválido (irregular), volta ao início IMEDIATAMENTE
    if (targetIndex === -1) {
        restartGame();
        return;
    }
    ...
    const delayTime = (jumpDistance === 2) ? 650 : 500;
    setTimeout(() => {
...
// Verifica se o jogo trancou (bloqueio tático) e reseta IMEDIATAMENTE
...
/* ♿ ACESSIBILIDADE GLOBAL (Aumenta/Diminui dinamicamente via rem) */
function changeFontSize(dir) {
    fontScale += (dir * 0.1);
    
    // Limites de segurança (mínimo de 0.7 e máximo de 1.6)
    if (fontScale < 0.7) fontScale = 0.7;
    if (fontScale > 1.6) fontScale = 1.6;
    
    // Aplica o tamanho na raiz (HTML), atualizando proporcionalmente todas as fontes em rem
...
// Inicializa o ecossistema do lago ao carregar o ficheiro
renderPond3D();